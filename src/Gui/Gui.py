...
class Gui:
    ...
    def draw(self):
        ...
        Radiobutton(self.ws, text="Usar 1 proxy por navegador", variable=self.opcion, 
                    ...
                    value=True, command=self.seleccionar).grid(row=3, column=1)

        Radiobutton(self.ws, text="Usar 1 proxy por 4 navegadores", variable=self.opcion, 

                    value=False, command=self.seleccionar).grid(row=4, column=1)
    ...
    def run(self):        
        ...
        bots = Setup(self.proxies_path,self.models_path,self.accounts_path,self.timeawaitval,self.oneproxybybrowser)
        bots.run()
